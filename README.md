...
suspend fun authenticate(activity: Activity, optionsJson: String) {
    ...
    try {
        ...
        when (error.reason) {
            ...
            AkedlyPasskeyNativeException.Reason.UNSUPPORTED,
            AkedlyPasskeyNativeException.Reason.INVALID_OPTIONS,
            AkedlyPasskeyNativeException.Reason.FAILED -> useHostedPasskeyFallback()
        }
    }
}
```
...
reports one. A coroutine `CancellationException` is rethrown so structured cancellation remains
intact.
