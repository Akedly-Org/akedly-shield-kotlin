...
The library keeps `minSdk = 24`. Building or consuming the native passkey API requires
`compileSdk` 35+, Android Gradle Plugin 8.6+, Kotlin 2.0+ for Kotlin consumers (the library emits
Kotlin 2.1 metadata), and JDK 17. The native path brings in AndroidX Credential Manager 1.6.0 and
its Google Play Services auth/base dependencies.
